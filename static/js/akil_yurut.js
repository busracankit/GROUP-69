const apiBase = "/exercises/akil-yurut";
...
async function fetchQuestions(level) {
    try {
        ...
        if (data.error || !data.questions || data.questions.length === 0) {
            ...
            // Hata durumunda başlangıç ekranına geri dön
        ...
        alert("Sorular getirilirken bir hata oluştu. Lütfen daha sonra tekrar deneyin.");
    }
}
...
async function handleAnswer(selectedAnswer, question) {
    ...
    // Her bir cevabı backend'e gönder (Yanlış cevapları kaydetmek için)
    const res = await fetch(`${apiBase}/submit?user_id=${encodeURIComponent(userId)}`, {
        method: "POST",
        credentials: "include", // Cookie ile gönderiliyor
        headers: {
...
async function saveExerciseSummary(summaryData) {
    try {
        const res = await fetch(`${apiBase}/complete`, { // Yeni /complete endpoint'i
            method: "POST",
            credentials: "include",
            headers: {
                "Content-Type": "application/json"
...
function showResults() {
    ...
    const summaryPayload = {
        ...
        correct_answers: correctAnswers, // Backend'de 'correct_answer' olarak karşılanacak
        wrong_answers: wrongAnswers      // Backend'de 'wrong_answer' olarak karşılanacak
    };
...
// Sayfa ilk yüklendiğinde sadece seviye seçim ekranını göster
window.onload = () => {
    document.getElementById("level-selection").style.display = "block";
    document.getElementById("exercise-container").style.display = "none";
    document.getElementById("results-screen").style.display = "none";
