const apiBase = "/exercises/harf-karistirma";
let currentQuestionIndex = 0;
let score = 0;
let questions = [];
let startTime = Date.now();
let exerciseInfo = {};
let userId = null; // Dinamik user_id, backend'den alınacak
...
async function fetchUserId() {
    try {
        const res = await fetch("/api/get", {
            method: "GET",
            credentials: "include"  // Cookie ile birlikte gönder
        });
        if (!res.ok) {
...
async function fetchQuestions(level) {
    try {
        ...
        alert("Sorular yüklenirken bir sorun oluştu. Lütfen daha sonra tekrar deneyin.");
    }
}
...
function loadQuestion() {
    ...
    startTime = Date.now(); // Her yeni soru için zamanlayıcıyı yeniden başlat
}
