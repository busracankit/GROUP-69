...
from app.models import Exercise,KodKiriciQuestion,UserActivity,UserExercise
import random
import string
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(KodKiriciQuestion).where(KodKiriciQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(KodKiriciQuestion).where(KodKiriciQuestion.id == payload["question_id"]))
