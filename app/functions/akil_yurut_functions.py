from app.models import Exercise,UserExercise,AkilYurutQuestion,UserActivity
import random
import string
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(AkilYurutQuestion).where(AkilYurutQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(AkilYurutQuestion).where(AkilYurutQuestion.id == payload["question_id"]))
