...
from app.models import Exercise,AnlamBagdastirQuestion,UserActivity,UserExercise
import random
import string
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(AnlamBagdastirQuestion).where(AnlamBagdastirQuestion.level == level))
    ...
    if not sampled_questions:
        return {"error": "Örnek soru seçilemedi."}
    ...
    if not exercise:
        return {"error": "İlişkili egzersiz bulunamadı."}
        ...
        "questions": [
            {
                ...
                "question_image": q.question_image,  # YENİ ALAN
                "option_a": q.option_a,
                "option_b": q.option_b,
                "option_c": q.option_c,
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(AnlamBagdastirQuestion).where(AnlamBagdastirQuestion.id == payload["question_id"]))
