...
from app.models import Exercise,AnlamSecQuestion,UserActivity,UserExercise
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(AnlamSecQuestion).where(AnlamSecQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(AnlamSecQuestion).where(AnlamSecQuestion.id == payload["question_id"]))
