...
from app.models import Exercise,NesneYonuTanimaQuestion,UserExercise,UserActivity
import random
import string
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(NesneYonuTanimaQuestion).where(NesneYonuTanimaQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(NesneYonuTanimaQuestion).where(NesneYonuTanimaQuestion.id == payload["question_id"]))
