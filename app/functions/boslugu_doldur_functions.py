...
from app.models import Exercise,BosluguDoldurQuestion,UserActivity,UserExercise
import random
import string
from sqlalchemy.future import select

async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(BosluguDoldurQuestion).where(BosluguDoldurQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(BosluguDoldurQuestion).where(BosluguDoldurQuestion.id == payload["question_id"]))
