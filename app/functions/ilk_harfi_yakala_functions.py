...
from app.models import Exercise,IlkHarfiYakalaQuestion,UserExercise,UserActivity
import random
import string
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(IlkHarfiYakalaQuestion).where(IlkHarfiYakalaQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(IlkHarfiYakalaQuestion).where(IlkHarfiYakalaQuestion.id == payload["question_id"]))
