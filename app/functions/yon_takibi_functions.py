...
from app.models import Exercise,YonTakibiQuestion,UserActivity,UserExercise
import random
import string
...
async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(YonTakibiQuestion).where(YonTakibiQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(YonTakibiQuestion).where(YonTakibiQuestion.id == payload["question_id"]))
