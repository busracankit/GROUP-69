...
from app.models import Exercise,HecelereAyirQuestion,UserActivity,UserExercise
import random
from sqlalchemy.future import select
import string

async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(HecelereAyirQuestion).where(HecelereAyirQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(HecelereAyirQuestion).where(HecelereAyirQuestion.id == payload["question_id"]))
