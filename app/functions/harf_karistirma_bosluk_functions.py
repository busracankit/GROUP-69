...
from app.models import Exercise,UserActivity,HarfKaristirmaBoslukQuestion,UserExercise
import random
from sqlalchemy.future import select


async def get_questions_by_level(db: AsyncSession, level: str):
    result = await db.execute(select(HarfKaristirmaBoslukQuestion).where(HarfKaristirmaBoslukQuestion.level == level))
...
async def submit_answer(db: AsyncSession, user_id: int, payload: dict):
    result = await db.execute(
        select(HarfKaristirmaBoslukQuestion).where(HarfKaristirmaBoslukQuestion.id == payload["question_id"]))
